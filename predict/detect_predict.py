...
class DetectPredict(object):
    ...
    def data_load(self, is_normal=True):
        ...
        pre_data = FaceTransData(path=self._data_path, data_dir=self._data_dir, batch_size=1,
                                 shuffle=True, num_works=2, is_normal=is_normal, log=self._log)
    ...
    def model_load(self):
        ...
        if self._args.module == 'vae':
            model = VAE(self._x_shape).to(self._device)
        elif self._args.module == 'cvae':
            model = CondiationalVAE(self._x_shape,64).to(self._device)
        else:
            model = AutoEncoder(self._x_shape).to(self._device)

        file_name = os.path.join(self._args.path, f'output/checkpoint/{self._args.module}-model_best.pth')
    ...
    def predict(self, is_normal=True):
        ...
        for batch_idx, (data, label) in enumerate(pre_data):
            ...
            if self._args.module == 'vae':
                ...
                loss,recons_loss = loss_function(decoded, data, mu, log_var, x_shape=self._x_shape)
            elif self._args.module == 'cvae':
                decoded, mu, log_var, encoded = model(data)
                loss, recons_loss, kl_loss = cvae_loss(decoded, data, mu, log_var, x_shape=self._x_shape)
            else:
                encoded, decoded = model(data)
                recons_loss = ae_loss(decoded, data, x_shape=self._x_shape)
            zip_list = [label.cpu().numpy().tolist()[0], recons_loss.cpu().detach().numpy().tolist()]
            result_list.append(zip_list)
...
def get_result(original_dir, trans_dir):
    ...
    predict = DetectPredict(args=args, data_path=data_path, data_dir=trans_dir, device=device, x_shape=512, log=log)
    ...
    df_result.to_csv(out_path +args.module+'_result.csv', index=None)
    df_result['photo_id'].to_csv(out_path + 'photo_id.txt', index=None)
    df_result['s_label'].to_csv(out_path + 's_label.txt', index=None)
